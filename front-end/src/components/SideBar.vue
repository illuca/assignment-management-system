<template>
  <div class="side-bar">
    <el-menu :default-active="routePath" class="side-bar-el-menu"
             text-color="#bfcbd9" active-text-color="#20a0ff"
             background-color="#3c4043" router>
          ...
          <el-submenu :index="item.index" :key="item.index">
            ...
            <template v-for="subItem in item.subs">
              <el-submenu :index="subItem.index" :key="subItem.index"
                          v-if="subItem.subs"></el-submenu>

              <template slot="title">{{ subItem.title }}</template>
              <el-menu-item :index="subSubItem.index" :key="i"
                            v-for="(subSubItem, i) in subItem.subs">
                {{ subSubItem.title }}
              </el-menu-item>
            </template>
          </el-submenu>
